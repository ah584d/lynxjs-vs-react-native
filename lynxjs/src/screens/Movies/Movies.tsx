...
export function Movies() {
  ...
  async function handleGetMovies() {
    ...
    if (useHighestRated) {
      ...
      const uniqueMovies = Array.from(new Set(freshMovies.map(movie => movie.id))).map(id => freshMovies.find(movie => movie.id === id));
        ...
        <view className='MainContent'>
          <view className='Header'>
            ...
            <view className='FilterSection'>
              <text className='FilterLabel'>{`${t('genre')}: ${GENRE_MAP?.[selectedGenre] || t('all')}`}</text>
              <view className='FilterOptions'>
                <view className={selectedGenre == null ? 'FilterButtonActive' : 'FilterButton'} bindtap={setSelectedGenre(null)}>
                  <text>{t('all')}</text>
                </view>
                <view className={selectedGenre == 28 ? 'FilterButtonActive' : 'FilterButton'} bindtap={setSelectedGenre(28)}>
                  <text>{t('action')}</text>
                </view>
                <view className={selectedGenre == 35 ? 'FilterButtonActive' : 'FilterButton'} bindtap={setSelectedGenre(35)}>
                  <text>{t('comedy')}</text>
                </view>
                <view className={selectedGenre == 18 ? 'FilterButtonActive' : 'FilterButton'} bindtap={setSelectedGenre(18)}>
                  <text>{t('drama')}</text>
                </view>
              </view>
            </view>
              ...
              <view className='FilterOptionsYear'>
                {Array(4)
                  ...
                  .map((_, i) => {
                    ...
                    return (
                      <view key={i} className={selectedYear == _keyItem ? 'FilterButtonYearActive' : 'FilterButtonYear'} bindtap={setSelectedYear(_keyItem)}>
